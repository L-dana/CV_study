...
#그레이 스케일로 이미지를 불러온다.
src =cv.imread('imgs/FT.png', cv.IMREAD_GRAYSCALE)

#픽셀값의 범위는 0~255 이므로 전체 막대 개수는 256개
histSize = 256

#픽셀값의 범위는 0~255 입니다. 상위 경계인 256은 포함하지 않는다.
histRange = (0, 256)

#히스토그램 막대 크기가 똑같고 처음 시작시 히스토그램이 비어있도록 만든다.
accumulate = False
...
#계산 범위(ranges) - 히스토그램을 계산할 범위(최소값~최대값 = 0~256)
#accumulate -  히스토그램 막대 크기가 똑같고 처음 시작 시 히스토그램이 비어있도록 한다.
gray_hist = cv.calcHist([src], [0], None, [histSize], histRange, accumulate)


#히스토그램을 보여줄 이미지를 생성한다.
hist_w = 256
hist_h = 400
histimg = np.zeros((hist_h, hist_w, 1), dtype=np.uint8)
...
#cv.normalize(gray_hist, gray_hist, alpha=0, beta=hist_h, normType=cv.NORM_MINMAX) << 옛날 방식, 작동 x
gh = cv.normalize(gray_hist, None, 0, hist_h, cv.NORM_MINMAX)


#히스토그램을 그려준다
for i in range(0, histSize):
    cv.line(histimg, (i, hist_h - int(np.round(gh[i]))), (i, hist_h - 0), (255, 255, 255), thickness=2)
